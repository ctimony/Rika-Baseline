...
class PodManager:
    ...
    def compute_opportunity_score(self, pod, alpha: float = 1.0) -> float:
        """
        Reward-Penalty Opportunity Score (normalized to [0,1]):
            score_sku = max(0, (w_class + scarcity) - alpha * redundancy) * space_ratio
            OS_pod = sum(score_sku) / OS_max

        Evaluated over ALL SKUs in the pod to measure whether the trip is worth it.
        A pod where many slots are depleted scores higher than one where only 1 slot is depleted.
        Triggered only when at least one picked SKU passes layer 1 (current_global <= rop_global).

        w_class:     A=0.6, B=0.3, C=0.1
        scarcity:    1 - (current_global / max_global)
        redundancy:  n_active / max(total_pods - 1, 1)  — fraction of OTHER pods with stock, in [0,1]
                     excludes pod itself; 0 when no alternatives exist → penalty disappears
        space_ratio: 1 - (pod_qty / limit_qty)
        OS_max:      sum(w_class + 1.0) over depleted SKUs only (space_ratio > 0)
                     avoids dilution from full slots — score reflects actual depletion state
        """
        CLASS_WEIGHT = {'A': 0.6, 'B': 0.3, 'C': 0.1}
        ...
        for sku, slot in pod.skus.items():
            ...
            w_class = CLASS_WEIGHT.get(data.get('item_class', 'C'), 0.1)
            ...
            sku_score = ((w_class + scarcity) - (alpha * redundancy)) * space_ratio
            total_score += max(0.0, sku_score)
            # os_max only from depleted slots — avoids dilution from full slots
            if space_ratio > 0:
                os_max += (w_class + 1.0)

        if os_max <= 0:
            return 0.0
        return total_score / os_max
