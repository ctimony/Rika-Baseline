...
class PodManager:
    ...
    def compute_opportunity_score(self, pod) -> float:
        # Gate entry: at least one SKU must be at or below rop_global
        has_triggering_sku = any(
            sku in self.skus_data
            and self.skus_data[sku]['current_global_qty'] <= self.skus_data[sku]['rop_global']
            for sku in pod.skus
        )
        if not has_triggering_sku:
            return 0.0

        # Evaluate all depleted SKUs in pod (not only those below rop_global)
        candidate_skus = [
        ...
        for sku in candidate_skus:
            ...
            rop = max(data.get('rop_global', 1), 1)
            ...
            # Coverage: total stock of this SKU across all other pods
            other_qty = sum(
                p.skus[sku]['current_qty']
                for p in self.sku_to_pods.get(sku, [])
                if p.pod_id != pod.pod_id
            )
            coverage_ratio = other_qty / rop
            redundancy_factor = 1.0 / (1.0 + coverage_ratio)

            total_score += stock_depletion * pod_depletion_ratio * redundancy_factor

        return total_score / len(candidate_skus)
